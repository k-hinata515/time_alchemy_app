import 'package:flutter/material.dart';
import 'package:time_alchemy_app/View/profile_edit_page.dart';

class Colors_compornet {
  static const globalBackgroundColorRed = Color(0xFF7E463E); //赤色
  static const globalBackgroundColorwhite = Color(0xFFF2E9E2); //薄ピンク
  static const inputFieldtextfontColor = Color(0xFFA98E87);
  static const sortTagColor = Color(0xFFBD9F97);
  static const textfontcolorocher = Color(0xFFA98E87);
  static const textfontColorBlack = Color(0xFf333333);
  static const textfontColorWhite = Color(0xFFFCFBFF);
  static const textfontColorGray = Color(0xFF7B7A7A);
  static const borderColorGray = Color(0xFFD4C9C9);
  static const hobbyDisplayWhite = Color(0xFFFAF9F4);
  static const selectedtagdefault = Color(0xFFD9D9D9);
  static const narrow_down = Color(0xFF654E48);
}
