...
class X_IconButton extends StatelessWidget{
   final String label;
    ...
    X_IconButton({required this.label, required this.imageAsset , required this.onPressed});  //表示する　テキスト、画像、画面遷移先を引数として使う
  @override
...
class Instagram_IconButton extends StatelessWidget{
   final String label;
    final String imageAsset;
    final GestureTapCallback onPressed;
    Instagram_IconButton({required this.label, required this.imageAsset , required this.onPressed});  //表示する　テキスト、画像、画面遷移先を引数として使う
  @override
  Widget build(BuildContext context) {
  final screen = ScreenRef(context).watch(screenProvider);
